...
修复 Claude Code 通过 API 聚合商（DMX 等）调用 DeepSeek 等第三方模型时，在 thinking 模式下触发的 400 错误：
...
```json
{
  ...
  "targets": ["deepseek"],
  "aliasMap": {},
  "stats": { "total": 259, "fixed": 183, "noop": 30, "untouched": 46, "errors": 0, "sseRewritten": 12 }
}
```
...
| 日志行 | 含义 |
|--------|------|
| `FIXED: injected N thinking block(s) [model=deepseek-v4-pro-guan-cc]` | 请求侧注入占位块 |
| `FIXED: injected N thinking block(s) [model=deepseek-...] (rewrote label -> real)` | 别名改写 + 注入 |
...
| `untouched [model=claude-opus-4-7]` | 非 deepseek，透传不修改 |
| `no-op [model=deepseek-v4-pro-guan-cc]` | deepseek 但无 tool_use，不注入 |
